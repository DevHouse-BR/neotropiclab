...
function ve_svg_sanitize_css_selectors( $selectors ) {
	...
	return apply_filters( 've_svg_sanitize_css_selectors', $sanitized );
}


//Allow SVG through WordPress Media Uploader
function ve_svg_cc_mime_types($mimes) {
		$mimes['svg'] = 'image/svg+xml';
		$mimes['svgz'] = 'image/svg+xml';
		return $mimes;
}
add_filter('upload_mimes', 've_svg_cc_mime_types');

/**
 * TEMP FIX FOR 4.7.1
 * Issue should be fixed in 4.7.2 in which case this will be deleted.
 */
function ve_svgs_disable_real_mime_check( $data, $file, $filename, $mimes ) {
...
add_filter( 'wp_check_filetype_and_ext', 've_svgs_disable_real_mime_check', 10, 4 );
...
function ve_svg_fix_svg_size_attributes( $out, $id ) {
...
add_filter( 'image_downsize', 've_svg_fix_svg_size_attributes', 10, 2 );


//Fixing SVG width and height attributes to show correctly in Media Library in grid mode
function ve_svg_prepare_attachment_for_js_filter($response, $attachment, $meta){
	if( $response['mime'] == 'image/svg+xml' && empty($response['sizes']) ){
		$svg_file_path = get_attached_file( $attachment->ID );

		$orig_size = ve_svg_get_original_svg_size( $svg_file_path );
...
function ve_svg_get_original_svg_size($file) {
	$arr = array();
...
add_filter('wp_prepare_attachment_for_js', 've_svg_prepare_attachment_for_js_filter', 10, 3);

//Define styles and scripts for site's front-end

function ve_svg_scripts() {
	wp_enqueue_script( 've_svg_js', plugins_url( '/js/svg.min.js', __FILE__ ), array(), VE_VERSION, true );
	
	wp_enqueue_script( 've_svg_js' );
}
add_action('wp_enqueue_scripts', 've_svg_scripts');
