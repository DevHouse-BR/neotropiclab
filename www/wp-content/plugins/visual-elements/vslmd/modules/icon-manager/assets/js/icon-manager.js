(function ($) {
    "use strict";
    $.vslmdTrigger = $.vslmdTrigger || {};
    $.vslmdTrigger.wp_media = $.vslmdTrigger.wp_media || [];
    $.vslmdTrigger.media_new = function () {
        var $body = $("body");
        $body.on('click', '.vslmd_upload_icon', $.vslmdTrigger.media_new_activate);
    };
    //intended for zip files only. if needed should be easy to expand in the future
    $.vslmdTrigger.media_new_activate = function (event) {
        ...
        file_frame.on('select update insert', function () {
            $.vslmdTrigger.media_new_insert(file_frame, options);
        });
        //open the media frame
        file_frame.open();
    };
    //insert the url of the zip file
    $.vslmdTrigger.media_new_insert = function (file_frame, options) {
    ...
    $(document).ready(function () {
        $.vslmdTrigger.media_new();
        //Fonts Zip file upload
        $("body").on('vslmd_insert_zip', $.vslmdTrigger.icon_insert);
        //font manager
        $("body").on('click', '.vslmd_del_icon', $.vslmdTrigger.icon_remove);
    });
    /************************************************************************
     EXTRA FUNCTIONS, NOT NECESSARY FOR THE DEFAULT UPLOAD
     *************************************************************************/
    $.vslmdTrigger.icon_insert = function (event, selection, options) {
        // clean the options field, we dont need to save a value
        options.input_target.val("");
        var manager = $('.vslmd_iconfont_manager');
        var msg = $('#msg');
        ...
        $.ajax({
            ...
            url: ajaxurl,
            data: {
                action: 'vslmd_ajax_add_zipped_font',
                values: selection,
            },
            beforeSend: function () {
                $('.spinner').css({
            ...
            success: function (response) {
                $('.spinner').hide();
                if (response.match(/vslmd_font_added/)) {
    ...
    $.vslmdTrigger.icon_remove = function (event) {
        event.preventDefault();
        var button = $(this),
            parent = button.parents('.vslmd-available-font:eq(0)'),
            manager = button.parents('.vslmd_iconfont_manager:eq(0)'),
            all_fonts = manager.find('.vslmd-available-font'),
        ...
        $.ajax({
            ...
            url: ajaxurl,
            data: {
                action: 'vslmd_ajax_remove_zipped_font',
                del_font: del_font,
            },
            beforeSend: function () {
                $('.spinner').css({
            ...
            success: function (response) {
                $('.spinner').hide();
                if (response.match(/vslmd_font_removed/)) {
